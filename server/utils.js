...
function write_temp_file(bytes, callback) {
    temp.open({prefix: "sdm_catalogue_service_", suffix: '.tmp'}, function(err, info) {
        if(err) {
            callback(err, null);
            return;
        }
...
module.exports = {
    ...
    log_debug: function(str) {
        console.log(util.format("SDM_CATALOGUE_SERVICE:DEBUG] %s", str));
    },
    log_error: function(str) {
        console.error(util.format("SDM_CATALOGUE_SERVICE:ERROR] %s", str));
    },
    log_info: function(str) {
        console.log(util.format("SDM_CATALOGUE_SERVICE:INFO] %s", str));
    },
